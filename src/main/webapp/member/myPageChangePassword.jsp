...
<script>
	function check(){
		...
		if(document.fm.password.value != document.fm.passwordCh.value){
			alert('비밀번호가 일치하지 않습니다.');
			return false;
		}
		
		document.fm.action="<%=request.getContextPath()%>/member/mypageChangePasswordAction.do";
    ...
    <section id="home">
    	<form name=fm>
    		<table class="myPassword">
    			...
    			<tbody>
					<tr>
	    				<td>변경 비밀번호 : <input type="password" name="password" required></td>
	    			</tr>
	    			<tr>
	    				<td>비밀번호 확인 : <input type="password" name="passwordCh" required></td>
	    			</tr>
	    			<tr>
	    				<td><button class="pSubBtn" type="button" onclick="check()">변경<button></td>
	    			</tr>
    			</tbody>
    		</table>
    	</form>
