...
/>
<script>
	function notifyModifySubmit(){
		document.fm.action="<%=request.getContextPath()%>/notify/notifyContentModifyAction.do?bidx=<%= nv.getBidx()%>";
...
<%} %>
    ...
    <section id="home">
		...
		<form name=fm>
			<thead>
				<tr>
					<th><input id="subject" name="subject" type="text" value="<%=nv.getSubject() %>" required></th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td><textarea id="content" name="content" required><%=nv.getContent() %></textarea></td>
				</tr>
			</tbody>
			<tfoot>
				<tr>
					<td>
						<button type="submit" onclick="notifyModifySubmit()">등록</button>
