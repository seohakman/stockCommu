<%@page import="stockCommu.domain.ReportVO"%>
...
<script type="text/javascript">
	function reportFn(){
		document.fm.action = "<%=request.getContextPath()%>/common/reportWriteAction.do?bidx=<%=rv.getBidx()%>&midx=<%=rv.getMidx()%>&board=<%=rv.getBoard()%>";
        ...
        <tr>
          ...
          <td>
			<select name="reason">
				...
				<option value="성인물/성적 모욕감">성인물/성적 모욕감</option>
				<option value="게시판 규정 미준수">게시판 규정 미준수</option>
				<option value="기타">기타</option>
			</select> 
		  </td>
        </tr>
		<tr>
          <td id="reportArea" colspan="2" > <textarea name="content" placeholder="자세한 내용을 작성해주세요." required></textarea> </td>
        </tr>
        <tr>	
          <td colspan="2">
            <button class="subBtn" type="submit" onclick="reportFn()">신고</button>
          </td>
        </tr>
      </table>
      </form>
